...
def save_data(df, database_filename):
    """Saves the DataFrame  to database path"""
    name = 'sqlite:///' + database_filename
    engine = create_engine(name)
    df.to_sql('Disasters_table', engine, index=False) 


def main():
    """Runs main functions which aims to Loads the data, cleans it and saves it in a database"""
    if len(sys.argv) == 4:
